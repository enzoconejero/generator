...
void generate_dmmy(t_language* language){
	...
	while(length_current < length_max){
		...
		if (elements_to_change == length_current){
			printf("Is the last word with %d length\n", length_current);
			length_current++;
...
// 	for (int i = 0; i < length_current; indexes[i] = 0, memcpy(word + i * size,	elements, size),++i);
...
// 		// printf("%d elements to change of %d\n", elements_to_change, length_current);
// 		if (elements_to_change == length_current){
// 			log_screen("Is the last word with %d length\n", length_current);
// 			length_current++;
